...
bool ModelListClass::Initialize(int numModels)
{
	...
	for (i = 0; i < m_modelCount; i++)
	{
		...
		m_ModelInfoList[i].positionX = (((float)rand() - (float)rand()) / RAND_MAX) * 10.0f + 128.0f;
		m_ModelInfoList[i].positionY = (((float)rand() - (float)rand()) / RAND_MAX) * 10.0f + 12.5f;
		m_ModelInfoList[i].positionZ = (((float)rand() - (float)rand()) / RAND_MAX) * 10.0f + 128.0f;
	}
